...
function action (globMatcher) {
	processCommand(globMatcher, this.outdir, console.log.bind(console));
}

function processCommand (globMatcher, outDir, logFunc) {
					...
					.done(function (fileObj) {
						fileObj.reqs.forEach(function (v) {
							if (v.shouldInject && self.success) {
								logFunc(fileObj.name + " | " + v.name);
							}
							else if (!v.shouldInject && self.error) {
								logFunc("404 | " + fileObj.name + " | " + v.name);
							}
						});
					});
			});
	...
	return new Promise(function (resolve, reject) {
		...
		for (var j = 0 ; j < modules.length ; j++) {
			...
			for (var k = 0 ; k < coreModules.length ; k++) {
				...
				else if (modules[j].endsWith("package.json")) {
					pushed = true;
					break;
				}
			}
